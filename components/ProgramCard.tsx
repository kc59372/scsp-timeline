...
export function ProgramCard({ program, events }: { program: Program; events: Milestone[] }) {
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-edge bg-panel p-6 transition-all duration-200 hover:border-indigo-500/40 hover:bg-[#162032]">
      {/* pre-header */}
      <div className="mb-2 flex items-center justify-between">
        <span className={`font-mono text-[0.7rem] font-bold uppercase tracking-[0.1em] ${style.text}`}>
          United States · {categoryLabel(program.category)}
        </span>
        <span className="flex items-center gap-2 font-mono text-[0.65rem] uppercase tracking-wide text-gray-500">
          <span className="rounded bg-indigo-500/10 px-2 py-0.5 text-indigo-300">Program</span>
      ...
      <h3 className="mb-2 text-lg font-semibold leading-snug text-gray-100">
        <Link href={`/program/${program.id}`} className="hover:text-indigo-300 hover:underline">
          {program.name}
        </Link>
      </h3>
      ...
      <ol className="flex flex-wrap items-stretch gap-2">
        {events.map((e, i) => (
          <li key={e.id} className="flex items-center gap-2">
            <Link
              href={`/system/${e.id}`}
              className="group flex min-w-[8.5rem] flex-col gap-1 rounded-md border border-edge bg-black/30 px-3 py-2 hover:border-indigo-500/50"
            >
              <span className="flex items-center gap-1.5">
                <span className={`h-2 w-2 shrink-0 rounded-full ${style.dot}`} />
                <span className="font-mono text-[0.65rem] font-semibold uppercase tracking-wide text-gray-300 group-hover:text-indigo-300">
              ...
              {e.contractValue != null && (
                <span className="font-mono text-[0.65rem] text-gray-400">{formatUsd(e.contractValue)}</span>
              )}
            </Link>
            {i < events.length - 1 && (
              <span className="font-mono text-gray-600" aria-hidden>
                →
              </span>
            )}
          </li>
      ...
      {/* footer */}
      {totalValue > 0 && (
        <div className="mt-4 flex items-center justify-end border-t border-edge pt-4 font-mono text-xs text-gray-400">
